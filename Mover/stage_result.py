"""
Mover/stage_result.py

The contract every Mover stage returns. Deliberate mirror of
Joiner/stage_result.py, it is copied rather than imported so the Mover pipeline
stays self-contained and a change to the Joiner can never reach it. The two
files carry the same shape; if one changes, change both.
...
class StageOutcome:
    ...
    The Mover path uses PROCEED / DUPLICATE / QUEUED / FAILED. HELD and SKIPPED
    are retained for contract parity with the Joiner but are unused here — the
    Mover has no hold queue (SoD removed per ADR-008, no governance gate that
    holds an event).
    """
    PROCEED   = "proceed"    # stage succeeded, continue to the next stage
    HELD      = "held"       # (unused on Mover) routed to a hold queue
    ...
    QUEUED    = "queued"     # conflict — another event is in progress for this employee
    SKIPPED   = "skipped"    # (unused on Mover) nothing to do
...
class StageResult:
    ...
    report_actions — audit-action entries the DRIVER appends to
                     ...
                     (action / package_id / detail / succeeded).
    report_warnings— plain strings the driver appends to audit_record["warnings"].
    hold_reasons   — unused on the Mover path; kept for contract parity.
